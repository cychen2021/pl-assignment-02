...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        validate_form(signature, 1)
        validate_formals(signature.rest)
        proc = LambdaProcedure(signature.rest, expressions.rest, env)
...
def do_define_macro(expressions, env):
    ...
    """
    validate_form(expressions, 2)
    pattern = expressions.first
    if isinstance(pattern, Pair) and scheme_symbolp(pattern.first):
        macro_name = pattern.first
        validate_formals(pattern.rest)
        macro_proc = MacroProcedure(pattern.rest, expressions.rest, env)
        env.define(macro_name, macro_proc)
        return macro_name
    else:
        bad_pattern = pattern.first if isinstance(pattern, Pair) else pattern
        raise SchemeError('non-symbol: {0}'.format(bad_pattern))
